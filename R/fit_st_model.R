...
if (process){
  ...
  samp2 <- predict_mle_mcmc(m1_simple, model = 2, mcmc_iter = 5000)
...
ggsave(knot_compare,
       filename = here::here("figs/knot_compare_fig.png"),
       dpi = 300,
       width = 6,
       height = 3.5)
...
spat_compare <- 
  ggplot() + 
  geom_line(data = x_spat_sub,
            aes(y = est, x = year,
                color = `Pred. region`)) +
  geom_point(data = x_spat_sub,
            aes(y = est, x = year,
                color = `Pred. region`)) +
  geom_ribbon(data = x_spat_sub,
              aes(x = year, ymin = lwr, ymax = upr,
                  fill = `Pred. region`), 
              alpha = 0.2) +
  scale_x_continuous(expand = c(0.01, 0.01)) +
...
ggsave(spat_compare,
       filename = here::here("figs/spat_compare_fig.png"),
       dpi = 300,
       width = 6,
       height = 3.5)
